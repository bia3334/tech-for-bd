/* Regenerate <site>/data/lessons.js from <site>/roadmap.json + the lesson HTML files.
   Run after editing any lesson:
     node tools/build-index.mjs bd             → Tech for BD
     node tools/build-index.mjs finance        → Finance for Tech
     node tools/build-index.mjs system-design  → System Design                    */
import { readFileSync, writeFileSync, readdirSync } from "node:fs";

const root = new URL("..", import.meta.url).pathname;
/* One site per directory: bd, finance, system-design. The root is only the landing page. */
const name = (process.argv[2] || "").replace(/\/$/, "");
if (!name) { console.error("usage: node tools/build-index.mjs <bd|finance|system-design>"); process.exit(1); }
const site = root + name + "/";
const { chapters, lessons: roadmap } = JSON.parse(readFileSync(site + "roadmap.json", "utf8"));
...
for (const entry of roadmap) {
  ...
  for (const m of html.matchAll(/<section id="(s\d+)">([\s\S]*?)<\/section>/g)) {
    ...
    docs.push({ id: `${entry.n}#${id}`, type: "section", lesson: entry.n, href: `${href}#${id}`,
                title: h2, subtitle: `Lesson ${entry.n} · ${label}`, text, tags });
  }
  ...
  literal("QUIZ").forEach((q, i) => docs.push({
    ...
    title: q.q, subtitle: `Lesson ${entry.n} · Test yourself`,
  ...
  [...html.matchAll(/<li><div><div class="say">([\s\S]*?)<\/div><div class="use">([\s\S]*?)<\/div>/g)]
    .forEach((m, i) => docs.push({
      ...
      title: strip(m[1]), subtitle: `Lesson ${entry.n} · Line worth stealing`,
  ...
  const lesson = { n: entry.n, href, title: strip(title), dek, minutes: facts[0] || entry.minutes || "",
                   ...
                   ch: entry.ch, status: "published", sections };
  lessons.push(lesson);
  docs.push({ id: entry.n, type: "lesson", lesson: entry.n, href, title: lesson.title,
              subtitle: `Lesson ${entry.n} · ${lesson.level}`,
...
for (const l of lessons.filter(l => l.status === "planned")) {
  docs.push({ id: l.n, type: "planned", lesson: l.n, href: "", title: l.title,
              subtitle: `Lesson ${l.n} · not written yet`, text: l.dek, tags: l.tags || [] });
}

const out = `/* GENERATED by tools/build-index.mjs — do not edit by hand. */\nwindow.LIB = ${JSON.stringify({ chapters, lessons, docs }, null, 1)};\n`;
writeFileSync(site + "data/lessons.js", out);
console.log(`${name}/data/lessons.js — ${chapters.length} chapters, ${lessons.length} lessons, ${docs.length} docs, ${(out.length / 1024).toFixed(1)} KB`);
