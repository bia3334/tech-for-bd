/* Regenerate data/lessons.js from tools/roadmap.json + the lesson HTML files.
   Run after editing any lesson:  node tools/build-index.mjs                  */
import { readFileSync, writeFileSync, readdirSync } from "node:fs";

const root = new URL("..", import.meta.url).pathname;
const roadmap = JSON.parse(readFileSync(root + "tools/roadmap.json", "utf8"));
const files = readdirSync(root + "lessons").filter(f => f.endsWith(".html"));
...
for (const entry of roadmap) {
  ...
  const html = readFileSync(root + "lessons/" + file, "utf8");
...
writeFileSync(root + "data/lessons.js", out);
console.log(`data/lessons.js — ${lessons.length} lessons, ${docs.length} docs, ${(out.length / 1024).toFixed(1)} KB`);
